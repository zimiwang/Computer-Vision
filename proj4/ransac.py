...
def find_inliers(x_0s, F, x_1s, threshold):
    ...
    errors = []
    errors_fundamental = fundamental_matrix.signed_point_line_errors(x_0s, F, x_1s)
    errors_fundamental_arr = np.array(errors_fundamental)
    errors_fundamental_arr_ab = np.abs(errors_fundamental_arr)

    total_error = (errors_fundamental_arr_ab[::2] + errors_fundamental_arr_ab[1::2])/2
    inliers = np.where(total_error <= threshold)[0]
...
def ransac_fundamental_matrix(x_0s, x_1s):
    ...
    inliers_x_0 = None
    inliers_x_1 = None
    num_iterations = calculate_num_ransac_iterations(.99, sample_size, .9)
    
    for _ in range(num_iterations):
        indices = np.random.choice(len(x_0s), sample_size, replace=False)
        sample_x_0s = x_0s[indices]
        sample_x_1s = x_1s[indices]
