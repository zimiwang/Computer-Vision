...
def signed_point_line_errors(x_0s, F, x_1s):
    ...
    x_0s, x_1s, F = np.array(x_0s), np.array(x_1s), np.array(F)
    ...
    for i in range(0, x_0s.shape[0]):
        errors.append(point_line_distance(F @ x_1s[i, :], x_0s[i, :]))
        errors.append(point_line_distance(F.transpose() @ x_0s[i, :], x_1s[i, :]))
