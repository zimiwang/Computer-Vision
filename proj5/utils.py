...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
    ...
    im_left = torch.randint(0, 2, (H, W, C), dtype=torch.float32)

    # Ensure the image is grayscale
    im_left = im_left.mean(dim=2, keepdim=True).repeat(1, 1, C)
    im_right = im_left.clone()

    block_H, block_W = H // 2, W // 2
    start_H, start_W = (H - block_H) // 2, (W - block_W) // 2

    shifted_block = im_right[start_H:start_H + block_H, start_W:start_W + block_W].clone()
    im_right[start_H:start_H + block_H, start_W - disparity:start_W + block_W - disparity] = shifted_block

    hole = torch.randint(0, 2, (block_H, disparity, C), dtype=torch.float32)
    hole = hole.mean(dim=2, keepdim=True).repeat(1, 1, C)
    im_right[start_H:start_H + block_H, start_W + block_W - disparity:start_W + block_W] = hole
